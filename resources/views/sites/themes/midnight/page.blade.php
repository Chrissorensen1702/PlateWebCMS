...
@section('content')
    @php
        ...
        $showHeaderCta = $bookingInHeader || ($header ? (bool) $header->show_cta : true);
        $headerCtaLabel = $bookingInHeader
            ? $site->resolvedBookingCtaLabel($header?->cta_label ?: 'Book tid')
            : ($header?->cta_label ?: 'Kontakt');
        $headerCtaHref = $bookingInHeader
            ? $bookingUrl
            : (\App\Support\Http\PublicSiteUrl::sanitize($header?->cta_href) ?: '/kontakt');
    @endphp
    ...
    @else
        <header class="{{ $headerClasses }}">
            <div class="ui-shell midnight-header">
                <a href="{{ route('sites.show', $site) }}" class="midnight-brand">
                    @if ($logoUrl)
                        <img src="{{ $logoUrl }}" alt="{{ $logoAlt }}" class="midnight-brand__logo">
                    @else
                        <span class="midnight-brand__mark"></span>
                    @endif
                ...
                <div class="midnight-header__actions">
                    @if ($navigation->isNotEmpty())
                        <nav class="midnight-nav">
                            @foreach ($navigation as $navPage)
                                ...
                                <a href="{{ $href }}" class="midnight-nav__link{{ $isCurrentPage ? ' midnight-nav__link--active' : '' }}">
                                    {{ $navPage->name }}
                                </a>
                            @endforeach
                        </nav>
                    ...
                    @if ($showHeaderCta && $headerCtaLabel && $headerCtaHref)
                        <a href="{{ $headerCtaHref }}" class="ui-button ui-button--accent midnight-header__cta" @if($bookingTarget) target="{{ $bookingTarget }}" rel="noreferrer" @endif>
                            {{ $headerCtaLabel }}
                        </a>
                    @endif
                </div>
