...
@section('content')
    @php
        ...
        $tagline = $showTagline ? ($header?->tagline ?: 'Beauty & Skin Studio') : null;
        ...
        $bookingHref = $page->is_home
            ? '#contact-block'
            : $siteHomeUrl.'#contact-block';
        ...
        $headerCtaLabel = $bookingInHeader
            ...
            : ($header?->cta_label ?: 'Book tid');
        $headerCtaHref = $bookingInHeader
            ? $bookingUrl
            : (\App\Support\Http\PublicSiteUrl::sanitize($header?->cta_href) ?: $bookingHref);
    @endphp
    ...
    @else
        <header class="{{ $headerClasses }}">
            <div class="ui-shell site-theme-header__inner">
                <a href="{{ $siteHomeUrl }}" class="site-theme-brand">
                    @if ($logoUrl)
                    ...
                    @if ($brandName || $tagline)
                        <span class="site-theme-brand__copy">
                            @if ($brandName)
                ...
                <div class="site-theme-header__actions">
                    ...
                    @if ($showHeaderCta && $headerCtaLabel && $headerCtaHref)
                        <a href="{{ $headerCtaHref }}" class="ui-button ui-button--accent site-theme-booking" @if($bookingTarget) target="{{ $bookingTarget }}" rel="noreferrer" @endif>
                            {{ $headerCtaLabel }}
                        </a>
                    @endif
                </div>
